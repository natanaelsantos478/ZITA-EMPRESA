    ...
    <div className="flex flex-col h-full bg-gray-950">
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-800 flex-shrink-0 bg-gray-900/60">
        ...
        <select
          ...
          className="px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-gray-300 focus:outline-none focus:border-brand-500"
        >
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody className="divide-y divide-gray-800/50">
            ...
            {filtered.map(agent => {
              ...
              // Extract links count from integracao_config
              const linksCount = ((agent.integracao_config as { links?: unknown[] } | undefined)?.links ?? []).length
                ...
                >
                  ...
                  <td className="px-3 py-3">
                    <div>
                      ...
                      {linksCount > 0 && (
                        <p className="text-xs text-gray-600 mt-0.5">{linksCount} link{linksCount > 1 ? 's' : ''}</p>
                      )}
                    </div>
                  </td>
                    ...
                    {agent.integracao_tipo ? (
                      <div>
                        ...
                        {agent.integracao_url && (
                          <p className="text-xs text-gray-700 truncate max-w-[140px]">{agent.integracao_url}</p>
                        )}
                      </div>
                    ) : (
