...
export default function Organograma() {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    return (localStorage.getItem('zita_view_mode') as ViewMode) ?? '2d'
  })
  ...
  // ── Connections for 2D view ───────────────────────────────────────────────
  const connections = agents
    .filter(a => a.organograma_parent_id && positions[a.organograma_parent_id] && positions[a.id])
    .map(a => ({ from: a.organograma_parent_id!, to: a.id, status: a.status }))
  ...
  const Toolbar = (
    <div className="absolute top-4 left-4 z-20 flex flex-col gap-1.5 bg-gray-900 border border-gray-800 rounded-xl p-2 shadow-lg">
      {/* View mode toggle */}
      <div className="flex flex-col gap-1 pb-1.5 mb-0.5 border-b border-gray-800">
        {[
          { mode: '3d'    as ViewMode, icon: Boxes,        title: '3D POV'    },
          { mode: '2d'    as ViewMode, icon: SquareDashed,  title: '2D Canvas' },
          { mode: 'lista' as ViewMode, icon: List,          title: 'Lista'     },
      ...
      {viewMode === '2d' && (
        <>
          <button onClick={() => setZoom(z => Math.min(1.5, z + 0.1))} title="Zoom in"
            className="p-2 rounded-lg hover:bg-gray-800 text-gray-400 hover:text-white transition-colors">
            <ZoomIn className="w-4 h-4" />
          </button>
          <button onClick={() => setZoom(z => Math.max(0.3, z - 0.1))} title="Zoom out"
            className="p-2 rounded-lg hover:bg-gray-800 text-gray-400 hover:text-white transition-colors">
            <ZoomOut className="w-4 h-4" />
          </button>
          <button onClick={centerAll} title="Centralizar"
            className="p-2 rounded-lg hover:bg-gray-800 text-gray-400 hover:text-white transition-colors">
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${
              showLines ? 'text-brand-400 bg-brand-500/10' : 'text-gray-400 hover:text-white hover:bg-gray-800'
            }`}
          >
  ...
  return (
    <div className="relative w-full h-[calc(100vh-56px)] overflow-hidden bg-gray-950">
        ...
        <>
          ...
          >
            <div style={{
              ...
              {showLines && (
                <svg style={{ position: 'absolute', top: 0, left: 0, width: '4000px', height: '3000px', pointerEvents: 'none' }}>
                  {connections.map(({ from, to, status }) => {
                    ...
                    const x1 = fp.x + NODE_W / 2; const y1 = fp.y + NODE_H
                    const x2 = tp.x + NODE_W / 2; const y2 = tp.y
                    const color = STATUS_COLOR[status] ?? '#6b7280'
                    const isOccupied = status === 'ocupada'
                    return (
                      <path key={`${from}-${to}`} d={makePath(x1, y1, x2, y2)}
                        fill="none" stroke={color} strokeWidth={isOccupied ? 2 : 1.5}
                        strokeOpacity={0.6} strokeDasharray={isOccupied ? '8,4' : undefined}>
                        {isOccupied && (
              ...
              {agents.length === 0 && (
                <div style={{ position: 'absolute', left: 200, top: 200, width: 400 }}
      ...
      {/* ── Sidebar lateral (shared across views) ────────────────────────── */}
